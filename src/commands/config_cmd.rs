...
pub fn execute(args: cli::ConfigArgs) -> Result<(), String> {
    ...
    if let Some(ref set_value) = args.set {
        ...
        match key {
            ...
            "build.target" => {
                config.build.target = if value.is_empty() {
                    None
                } else {
    ...
    // Show current config
    let project_dir = RustmConfig::find_project_root().ok_or(
        "Not in a Rust project directory. Run 'rustm config --generate' to create one.".to_string(),
    )?;
